resource "azurerm_kubernetes_cluster" "aks_cluster" {
  ...
  lifecycle {
    # cluster_version is null, so AKS picks the default and auto-upgrades it.
    # Without this, every later plan wants to drag the cluster back, and with
    # auto-apply in CI that becomes an unattended control-plane change.
...
# Azure RBAC is eventually consistent: the grant above is visible to ARM's
# authorization cache anywhere from seconds to several minutes later. Installing
# ingress-nginx inside that window is what produces
# "AuthorizationFailed ... virtualNetworks/subnets/read" on the ingress subnet,
# even though the role assignment plainly exists. Hold the apply open until the
# grant has settled so anything running after Terraform is safe.
resource "time_sleep" "wait_for_network_rbac" {
